...
public class LevelClearScreen {
    ...
    public LevelClearScreen(Runnable onNextLevel, Runnable onRetry, Runnable onMenu) {
        ...
        Text title = new Text("LEVEL CLEARED!");
        ...
        StackPane nextBtn = createSpriteBtn("NEXT LEVEL", buttonSheet, 3, 0.5, onNextLevel);
        StackPane retryBtn = createSpriteBtn("RETRY", buttonSheet, 3, 0.5, onRetry);
        StackPane menuBtn = createSpriteBtn("MAIN MENU", buttonSheet, 3, 0.5, onMenu);

        VBox buttonBox = new VBox(0, nextBtn, retryBtn, menuBtn);
        buttonBox.setAlignment(Pos.CENTER);

        VBox uiBox = new VBox(25, title, buttonBox);
    ...
    private StackPane createSpriteBtn(String btnText, Image spriteSheet, int numFrames, double scaleMultiplier, Runnable action) {
        ...
        Font pixelFont = Font.loadFont(getClass().getResourceAsStream("/fonts/PixelFont.ttf"), 24);
        if (pixelFont == null) {
            pixelFont = Font.font("Arial", FontWeight.BOLD, 24);
        }
