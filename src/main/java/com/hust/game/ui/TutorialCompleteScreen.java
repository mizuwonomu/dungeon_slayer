...
public class TutorialCompleteScreen {
    private final StackPane root;

    public TutorialCompleteScreen(Runnable onStartLevel1, Runnable onMenu) {
        ...
        Text title = new Text("TUTORIAL COMPLETE");
        ...
        StackPane startBtn = createSpriteBtn("START LEVEL 1", buttonSheet, 3, 0.6, onStartLevel1);
        StackPane menuBtn = createSpriteBtn("MAIN MENU", buttonSheet, 3, 0.6, onMenu);

        VBox buttonBox = new VBox(15, startBtn, menuBtn);
        buttonBox.setAlignment(Pos.CENTER);

        VBox uiBox = new VBox(40, title, buttonBox);
    ...
    private StackPane createSpriteBtn(String btnText, Image spriteSheet, int numFrames, double scaleMultiplier, Runnable action) {
        ...
        if (pixelFont == null) {
            pixelFont = Font.font("Arial", FontWeight.BOLD, 28);
        }
